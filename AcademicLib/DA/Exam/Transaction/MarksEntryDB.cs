...
namespace AcademicLib.DA.Exam.Transaction
{
    internal class MarksEntryDB
    {
        ...
        public ResponeValues PublishedExamResult(int UserId,int AcademicYearId, int ExamTypeId,string ClassIdColl,int? BatchId,int? SemesterId,int? ClassYearId)
        {
            ResponeValues resVal = new ResponeValues();
 ...
 /*           cmd.Parameters.AddWithValue("@BatchId", BatchId);
        ...
        public AcademicLib.API.Student.MarkSheetTemplate getMarkSheetTemplateTranId(int UserId,int AcademicYearId)
        {
            ...
            try
            {
                ...
                {                    
                    ...
                    if (!(reader[2] is DBNull)) beData.AdmitCardId = reader.GetInt32(2);
                }
                reader.Close();                
        ...
        public AcademicLib.RE.Exam.StudentResultCollections getStudentResult(int UserId,int AcademicYearId, int StudentId)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (!(reader[10] is DBNull)) beData.ObtainTH = Convert.ToDouble(reader[10]);
                    if (!(reader[11] is DBNull)) beData.ObtainPR = Convert.ToDouble(reader[11]);
                    if (!(reader[12] is DBNull)) beData.IsFail = Convert.ToBoolean(reader[12]);
                    if (!(reader[13] is DBNull)) beData.IsFailPR = Convert.ToBoolean(reader[13]);
                    if (!(reader[14] is DBNull)) beData.IsFailTH = Convert.ToBoolean(reader[14]);
        ...
        public ResponeValue IsValidForMarkSheetPrint(int UserId,int? StudentId, int? ExamTypeId,int? ExamTypeGroupId,int? AcademicYearId)
        {
            ResponeValue resVal = new ResponeValue();
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@ExamTypeGroupId", ExamTypeGroupId);
                cmd.Parameters.AddWithValue("@AcademicYearId", AcademicYearId);
                cmd.CommandText = "usp_IsValidStudentForMarkSheetPrint";
                cmd.ExecuteNonQuery();
                if (!(cmd.Parameters[2].Value is DBNull))
                    resVal.ResponseMSG = Convert.ToString(cmd.Parameters[2].Value);
        ...
        public ResponeValue IsValidForAdmitCardPrint(int UserId, int? StudentId, int? ExamTypeId, int? ExamTypeGroupId, int? AcademicYearId)
        {
            ResponeValue resVal = new ResponeValue();
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@ExamTypeGroupId", ExamTypeGroupId);
                cmd.Parameters.AddWithValue("@AcademicYearId", AcademicYearId);
                cmd.CommandText = "usp_IsValidStudentForAdmitCardPrint";
                cmd.ExecuteNonQuery();
                if (!(cmd.Parameters[2].Value is DBNull))
                    resVal.ResponseMSG = Convert.ToString(cmd.Parameters[2].Value);
