...
namespace PivotalERP.Areas.Inventory.Controllers
{
    public class ReportingController : PivotalERP.Controllers.BaseController
    {
        ...
        [HttpPost]
        public JsonNetResultWithEnum GetSalesInvoiceDetails(DateTime? dateFrom, DateTime? dateTo, bool ShowReturn = false, bool ShowDebitNote = false, bool ShowCreditNote = false)
        {
            ...
            try
            {
                ...
                dataColl = new Dynamic.Reporting.Inventory.SalesRegister(User.HostName, User.DBName).getSalesInvoiceDetails(User.UserId, dateFrom.Value, dateTo.Value, ShowReturn, ShowDebitNote, ShowCreditNote);
                return new JsonNetResultWithEnum() { Data = dataColl, TotalCount = dataColl.Count, IsSuccess = dataColl.IsSuccess, ResponseMSG = dataColl.ResponseMSG };
            }
            catch (Exception ee)
            {
        ...
        {
            Dynamic.ReportEntity.Inventory.SalesAnalysisProductWiseCollections dataColl = new Dynamic.Reporting.Inventory.PurchaseAnalysis(User.HostName, User.DBName).getPurchaseAnalysisProductWise(User.UserId, dateFrom, dateTo);
        ...
        public JsonNetResultWithEnum GetPendingPurchaseOrder(DateTime dateFrom, DateTime dateTo,int ReportTypeId)
        {
            ...
            Dynamic.ReportEntity.Inventory.PendingSalesOrderDetailsCollections dataColl = new Dynamic.ReportEntity.Inventory.PendingSalesOrderDetailsCollections();// new Dynamic.Reporting.Inventory.PendingPurchaseOrder(User.HostName, User.DBName).getPendingSalesDetails(User.UserId, dateFrom, dateTo, isPendingOnly, isClearOnly);
        ...
        [HttpPost]
        public JsonNetResultWithEnum GetProductVoucher(DateTime dateFrom, DateTime dateTo, int productId, string godownIdColl, string BatchColl, string EngineNoColl)
        {
            Dynamic.BusinessEntity.Setup.CompanyDetail comDet = new Dynamic.DataAccess.Setup.CompanyDetailDB(User.HostName, User.DBName).getCompanyDetailsForWeb(User.UserId);
            ...
            DateTime openingDateAD = comDet.StartDate;
            string openingDateBS = "";
            double openingAmt = 0;
            double openingQty = 0;
            double closingRate = 0;
            Dynamic.ReportEntity.Account.StockReport stockReport = new Dynamic.ReportEntity.Account.StockReport();
            ...
            var tmpColl = new Dynamic.Reporting.Inventory.ProductMonthlySummary(User.HostName, User.DBName).getProductVoucher(ref openingColl, ref stockReport, User.UserId, productId, dateFrom, dateTo, ref openingQty, ref openingAmt, ref closingRate, ref openingDateAD, ref openingDateBS, false, godownIdColl, BatchColl, EngineNoColl);
            ...
            if (openingAmt != 0 || openingQty != 0)
            {
                ...
                voucher.VoucherDate = openingDateAD;
                voucher.VoucherDateNP = openingDateBS;
            ...
            if (stockReport != null)
            {
                clQty = stockReport.B_Qty;
                clAmt = stockReport.B_Amt;
                clRate = stockReport.B_Rate;
            }
            else
            {
                clQty = inQty - outQty;
        ...
        [HttpPost]
        public JsonNetResultWithEnum GetStockSummary(int ProductGroupId, DateTime dateFrom, DateTime dateTo, int? GodownId, bool GodownWise, int? AUnitId1, int? AUnitId2, int? AUnitId3, bool BatchWise, string GodownIdColl, bool ShowStandardRate,
            int? ProductCompanyId = null, int? ProductCategoryId = null, int? ProductTypeId = null
            )
        {
            Dynamic.ReportEntity.Account.StockReportCollections dataColl = new Dynamic.ReportEntity.Account.StockReportCollections();
            try
            {
                dataColl = new Dynamic.Reporting.Account.StockReport(User.HostName, User.DBName).getStockReports(User.UserId, ProductGroupId, dateFrom, dateTo, GodownId, GodownWise, AUnitId1, AUnitId2, AUnitId3, BatchWise, GodownIdColl, ShowStandardRate, ProductCompanyId, ProductCategoryId, ProductTypeId);
