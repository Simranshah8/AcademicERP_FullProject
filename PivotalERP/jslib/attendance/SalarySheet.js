...
app.controller('SalarySheetController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	//Added By Suresh on 24 Mangsir
	$scope.bdet = {};
	$scope.GetTaxCalcDetail = function (ref) {
		$scope.bdet.EmployeeName = ref.EmployeeName;
		$scope.bdet.EmployeeCode = ref.EmployeeCode;
		$scope.bdet.YearId = ref.YearId;
		var monthObj = $scope.MonthList.find(function (m) {
			return m.id === ref.MonthId;
		});

		// Assign MonthName safely
		$scope.bdet.MonthName = monthObj ? monthObj.text : '';
		$('#GetTaxCalcDetail').modal('show');
