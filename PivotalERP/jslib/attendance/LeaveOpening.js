app.controller('LeaveOpeningController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.IsValidLeaveOpening = function () {
		...
		for (var i = 0; i < $scope.newLeaveOpening.LeaveQuotaDetail.length; i++) {

			if ($scope.newLeaveOpening.LeaveQuotaDetail[i].NoOfLeave < 0) {
				Swal.fire('Balance cannot be negative for ' + $scope.newLeaveOpening.LeaveQuotaDetail[i].Name);
				return false;
			}
		}
		return true;
