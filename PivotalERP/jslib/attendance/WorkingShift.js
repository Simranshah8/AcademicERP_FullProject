app.controller('WorkingShiftController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.IsValidCreate = function () {
		...
		if ($scope.newWorkingShift.Break1Duration != null && $scope.newWorkingShift.Break1Duration < 0) {
			Swal.fire('Break 1 Duration Cannot be Negative');
			return false;
		}
		if ($scope.newWorkingShift.Break2Duration != null && $scope.newWorkingShift.Break2Duration < 0) {
			Swal.fire('Break 2 Duration Cannot be Negative');
			return false;
		}
		if ($scope.newWorkingShift.HalfDayDuration != null && $scope.newWorkingShift.HalfDayDuration < 0) {
			Swal.fire('Half Day Duration Cannot be Negative');
			return false;
		}
		return true;
	}
	...
	$scope.IsValidMapping = function () {
		...
		var fromDate = new Date($scope.newMapping.DateFromDet.dateAD);
		var toDate = new Date($scope.newMapping.DateToDet.dateAD);
