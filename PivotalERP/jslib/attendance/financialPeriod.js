app.controller('FinancialPeriodController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.IsValidFinancialPeriod = function () {
		if (!$scope.newFinancialPeriod.Name || $scope.newFinancialPeriod.Name.trim() === '') {
			Swal.fire('Please Enter Financial Period Name');
			return false;
		}
		if (!$scope.newFinancialPeriod.StartDate_TMP || !$scope.newFinancialPeriod.EndDate_TMP) {
			Swal.fire('Please select both Start Date and End Date');
			return false;
		}
		var startDate = new Date($scope.newFinancialPeriod.StartDate_TMP);
		var endDate = new Date($scope.newFinancialPeriod.EndDate_TMP);
