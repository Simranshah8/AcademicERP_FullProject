...
app.controller('QuestionCategoryController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.IsValidQuestionCategory = function () {
		...
		if ($scope.newQuestionCategory.OrderNo == null ||
			isNaN($scope.newQuestionCategory.OrderNo) ||
			$scope.newQuestionCategory.OrderNo <= 0) {
			Swal.fire('Please enter a valid Order No (greater than 0)');
			return false;
		}
