app.controller('EvaluateController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.GetClassWiseSubMap = function () {
		...
		$scope.newEvaluate.SubjectList = [];

		if ($scope.newEvaluate.ClassId && $scope.newEvaluate.ClassId > 0) {
			var para = {
				ClassId: $scope.newEvaluate.ClassId,
				SectionIdColl: ($scope.newEvaluate.SectionId ? $scope.newEvaluate.SectionId.toString() : '')
			};
					...
					if (SubjectMappingColl.length == 0) {
						...
						angular.forEach(SubjectMappingColl, function (sm) {
							...
							if (subDet) {
								...
								subDet.OTH = 1;
								subDet.OPR = 1;
								$scope.newEvaluate.SubjectList.push(subDet);
							}
						});
					}
				} else {
	...
	$scope.GetAllExamList = function () {
		...
		if ($scope.newEvaluate.ClassId && $scope.newEvaluate.SubjectId && $scope.newEvaluate.ExamTypeId) {
			...
			$http({
				method: 'POST',
				url: base_url + "elearning/creation/GetOnlineExamListForEvaluate",
				dataType: "json",
				data: JSON.stringify(para)
			}).then(function (res) {
				hidePleaseWait();
	...
	$scope.ExamCopyCheck = function () {
		...
		if ($scope.CurrentQuestionForCheck) {
			...
			$http({
				method: 'POST',
				url: base_url + "elearning/creation/ExamCopyCheck",
