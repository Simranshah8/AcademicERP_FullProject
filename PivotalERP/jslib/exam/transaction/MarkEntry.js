...
app.controller('MarkEntryController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.GetClassWiseSubMap = function (fromInd) {
		...
		if ($scope.newClassWise.SelectedClass) {
			...
			var para = {
				...
				BranchId: $scope.GetBranchId($scope.newClassWise.BranchId),
			};
			...
			$http({
				...
				if (res.data.IsSuccess && res.data.Data) {
					...
					else if (SubjectMappingColl.length > 0)
					{
						...
						if ($scope.newClassWise.ExamTypeId && $scope.newClassWise.ExamTypeId > 0) {
							...
							$http({
								method: 'POST',
								url: base_url + "Exam/Transaction/GetStudentForClassWiseME",
								dataSchedule: "json",
									...
									angular.forEach($scope.newClassWise.StudentColl, function (st) {
										...
										angular.forEach($scope.newClassWise.SubjectList, function (sl) {
											...
											var om = {
												...
												SubjectRemarks: (fSub ? fSub.SubjectRemarks : ''),
												SubjectType: (fSub ? fSub.SubjectType : 0),
												OTH: (fSub ? fSub.OTH : 0),
												OPR: (fSub ? fSub.OPR : 0),
												IsInclude: (fSub ? fSub.IsInclude : 0),
											};

											if (!isNaN(parseFloat(om.ObtainMarkTH)))
											{
												var oth = parseFloat(om.ObtainMarkTH);
												var fth = parseFloat(om.PMTH);
												if (oth < fth) {
													om.IsFailTH = true;
												} else
													om.IsFailTH = false;
											...
											if (!isNaN(parseFloat(om.ObtainMarkPR))) {
												var oth = parseFloat(om.ObtainMarkPR);
												var fth = parseFloat(om.PMPR);
												if (oth < fth) {
													om.IsFailPR = true;
												} else
													om.IsFailPR = false;
	...
	$scope.DelClassWiseById = function (refData) {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				$http({
					method: 'POST',
					url: base_url + "Exam/Transaction/DelClassWise",
					dataType: "json",
					data: JSON.stringify(para)
				}).then(function (res) {
					hidePleaseWait();
	...
	$scope.GetClassWiseSubMapSME = function () {
		...
		if ($scope.newSubjectWise.SelectedClass) {
			var para = {
				...
				BranchId: $scope.GetBranchId($scope.newSubjectWise.BranchId),
				//Added on 1 Poush
	...
	$scope.GetMarkEntrySubjectWise = function () {
		...
		$scope.newSubjectWise.StudentColl = [];
		if ($scope.newSubjectWise.SelectedClass && $scope.newSubjectWise.ExamTypeId && $scope.newSubjectWise.SubjectId) {
			var para = {
				ClassId: $scope.newSubjectWise.SelectedClass.ClassId,
				...
				FilterSection: $scope.newSubjectWise.SelectedClass.FilterSection,
				BranchId: $scope.GetBranchId($scope.newSubjectWise.BranchId),
				//Added on 1 Poush
				SemesterId: $scope.newSubjectWise.SemesterId || null,
				ClassYearId: $scope.newSubjectWise.ClassYearId || null,
			...
			$http({
				method: 'POST',
				url: base_url + "Exam/Transaction/GetStudentForSubjectME",
				dataSchedule: "json",
				data: JSON.stringify(para)
			}).then(function (res1) {
	...
	$scope.CallSaveUpdateSubjectWise = function () {
		...
		angular.forEach($scope.newSubjectWise.StudentColl, function (st) {
			var ms = {
				...
				SubjectRemarks:st.SubjectRemarks
			};
			tmpDataColl.push(ms);
		});
		$http({
	...
	$scope.DelSubjectWiseById = function (refData) {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				var para = {
					SubjectWiseId: refData.SubjectWiseId
				};

				$http({
					method: 'POST',
					url: base_url + "Exam/Transaction/DelSubjectWise",
					dataType: "json",
					data: JSON.stringify(para)
				}).then(function (res) {
					hidePleaseWait();
	...
	$scope.GetStudentListForME = function () {
		...
		var para = {
			...
			BranchId: $scope.GetBranchId($scope.newStudentWise.BranchId),
			//Added on 1 Poush
	...
	$scope.GetSubjectForSWME = function () {
		if ($scope.newStudentWise.StudentId && $scope.newStudentWise.ExamTypeId && $scope.newStudentWise.ExamTypeId > 0)
		{
			var para1 = {
			...
			$http({
				method: 'POST',
				url: base_url + "Exam/Transaction/GetStudentForStudentWiseME",
				dataSchedule: "json",
				data: JSON.stringify(para1)
			}).then(function (res) {
				if (res.data.IsSuccess && res.data.Data) {
	...
	$scope.GetAllMarkSubmittedStatusList = function () {
		...
		if ($scope.newMarkSubmittedStatus && $scope.newMarkSubmittedStatus.ExamTypeId) {
			...
			var para = {
				...
				BranchId: $scope.GetBranchId($scope.newMarkSubmittedStatus.BranchId),
			};
			$http({
				method: 'POST',
