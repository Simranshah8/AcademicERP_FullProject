...
app.controller('SiblingFeeReceiptController', function ($scope, $http, $timeout, $filter, $rootScope, GlobalServices, $translate, uiGridConstants, uiGridTreeViewConstants) {
	...
	$scope.getDuesDetails = function () {
		...
		if ($scope.SelectedCostClass && $scope.newFeeReceipt.StudentId && $scope.newFeeReceipt.StudentId > 0) {
			...
			GlobalServices.getAcademicMonthList($scope.newFeeReceipt.StudentId, null).then(function (resAM) {
				...
				$http({
					method: 'POST',
					url: base_url + "Fee/Transaction/GetDuesForSiblingReceipt",
					dataType: "json",
					data: JSON.stringify(para)
				}).then(function (res) {
					if (res.data.IsSuccess && res.data.Data) {
						...
						angular.forEach($scope.CurDues.FeeItemWiseDuesColl, function (fd) {
							fd.StudentDet = mx($scope.CurDues.SiblingStudentColl).firstOrDefault(p1 => p1.StudentId == fd.StudentId);
						});
	...
	$scope.CalculationOnTotal = function (col) {
		if (col == 1) {
			...
			var totalAmt = $scope.newFeeReceipt.ReceivedAmt;
	...
	function getterAndSetter() {
		...
		$scope.gridFRCollection = {
			...
			{ name: "AutoVoucherNo", displayName: "Rec.No", minWidth: 120, headerCellClass: 'headerAligment' },
			{
