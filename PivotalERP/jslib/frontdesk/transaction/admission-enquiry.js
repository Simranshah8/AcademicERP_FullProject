...
app.controller('EnquiryController', function ($scope, $http, $timeout, $filter, $rootScope, $translate, GlobalServices, FileSaver) {
	...
	$scope.IsValidEnquiry = function () {
		...
		if ($scope.ComDet.Country == 'Nepal') {
			if (!$scope.newEnquiry.LastName || $scope.newEnquiry.LastName.trim() === '') {
				Swal.fire('Please ! Enter Last Name');
				return false;
			}
		}

		if (!$scope.newEnquiry.Gender) {
			Swal.fire('Please ! Select Gender');
			return false;
		}

		//if (!$scope.newEnquiry.CA_FullAddress || $scope.newEnquiry.CA_FullAddress.isEmpty()) {
		//	Swal.fire('Please ! Enter Current Full Address');
		//	return false;
		//}
	...
	$scope.CallSaveUpdateEnquiry = function () {
		...
		if ($scope.newEnquiry.DOBADDet && $scope.newEnquiry.DOBADDet.dateAD) {
			$scope.newEnquiry.DOB = $filter('date')(new Date($scope.newEnquiry.DOBADDet.dateAD), 'yyyy-MM-dd');
		}
	...
	$scope.updateDOB = function (dateStyle) {
		$timeout(function () {
			if (dateStyle == 1) {
				if ($scope.newEnquiry.DOBADDet) {
					$scope.newEnquiry.DOB_TMP = new Date($scope.newEnquiry.DOBADDet.dateAD);
				}
				else {
					$scope.newEnquiry.DOB_TMP = null;
				}
			}
			else if (dateStyle == 2) {
				if ($scope.newEnquiry.DOBDet) {
					$scope.newEnquiry.DOBAD_TMP = new Date($scope.newEnquiry.DOBDet.dateAD);
				}
				else {
					$scope.newEnquiry.DOBAD_TMP = null;
				}
			}
		});
	};
