...
app.controller('TermSyllabusController', function ($scope, $http, $timeout, GlobalServices) {
    ...
    $scope.GetClassWiseSubjectList = function () {
        $scope.SubjectList = [];
        var para = {
            BatchId: $scope.newSyllabus.BatchId ||null,
            ClassId: $scope.newSyllabus.SelectedClass.ClassId,
            ClassYearId: $scope.newSyllabus.ClassYearId || null,
            SemesterId: $scope.newSyllabus.SemesterId || null,
        };
        $http({
            method: 'POST',
    ...
    $scope.IsValidSyllabus = function () {
        ...
        if (!$scope.newSyllabus.SelectedClass.ClassId) {
    ...
    $scope.GetAllSyllabusList = function () {
        $http({
            ...
            if (res.data.IsSuccess && res.data.Data) {
                ...
                var grouped = {};
                var subjectTracker = {}; // Track unique subjects

                angular.forEach(rawData, function (item) {
                    // Create unique key for subject (without topic differentiation)
                    var subjectKey = item.ExamTypeId + '_' + item.ClassId + '_' +
                        (item.SemesterId || 0) + '_' + (item.ClassYearId || 0) + '_' +
                        item.SubjectId;

                    // Check if this subject is already processed
                    if (!subjectTracker[subjectKey]) {
                        subjectTracker[subjectKey] = true;

                        var groupKey = item.ExamTypeId + '_' + item.ClassId + '_' +
                            (item.SemesterId || 0) + '_' + (item.ClassYearId || 0);
                        ...
                        if (!grouped[groupKey]) {
                            grouped[groupKey] = {
                                ...
                                Subjects: []  // Array to hold unique subjects
                            };
                        }
                ...
                if (res.data.IsSuccess === false && res.data.ResponseMSG) {
                    Swal.fire(res.data.ResponseMSG);
                }
            }
        }, function (reason) {
            Swal.fire('Failed to load syllabus: ' + reason);
            $scope.GroupedSyllabusList = [];
        });
    };
    ...
    $scope.ShowClasswiseSyllabus = function (S) {
        ...
        $scope.ClassYear = S.ClassYear;
        $scope.Semester = S.Semester;
        if (S.SubjectId != null) {
            $scope.loadingstatus = "running";
            showPleaseWait();
    ...
    $scope.GetClassSubjectWiseTermSyllabus = function () {
        ...
        if ($scope.newSyllabus.SubjectId != null && $scope.newSyllabus.ExamTypeId != null) {
            $scope.loadingstatus = "running";
            showPleaseWait();
